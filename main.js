  ...
  rs(url, (error, response, html) => {
    if (!error) {
      try {
        ...
        res.status(404).json({ e: "404 not found" });
      }
    }
  });
});
      ...
      try {
        ...
        $("p.type").each(function (index, element) {
          if ("Type: " == $(this).children("span").text()) {
            ...
            status = $(this).text().slice(8).split('/n').join('').trim();
          } else if ("Genre: " == $(this).children("span").text()) {
            genresTemp = $(this).text().slice(20, -4);
            genres = genresTemp.split(", ");
      ...
      try {
        ...
        $("ul.listing")
          ...
          .each(function (index, element) {
            ...
            let image = regex.exec($(this).attr().title) || ["", ""]

            list[index] = { title, id, image: image[1] };
          });
